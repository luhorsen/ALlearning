...
with open(
    "/home/zsc/Yf/ML/ActivateLearning/Rumor/codes/AL_Comparision/rumdect.json",
    "r",
) as file:
    data = json.load(file)
features = []
...
alibox = ToolBox(X=X, y=y, query_type="AllLabels", saving_path="./RBF_QBC")
...
for round_exp in range(2):
    ...
    # )
    model = SVC(
        kernel="rbf", gamma="scale", random_state=random_state_arry2[round_exp]
    )
...
analyser.plot_learning_curves(
    title="Example of alipy", std_area=False, saving_path="./RBF_QBC"
)
