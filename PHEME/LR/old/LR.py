...
with open(
    "/home/zsc/Yf/ML/ActivateLearning/Rumor/codes/PHEME/pheme.json",
    "r",
) as file:
    data = json.load(file)
features = []
...
alibox = ToolBox(X=X, y=y, query_type="AllLabels", saving_path="./Unc_Entropy")

# Split data
alibox.split_AL(
    test_ratio=0.3, initial_label_rate=0.007, split_count=10, all_class=True
)
...
uncertainStrategy = alibox.get_query_strategy(
    strategy_name="QueryInstanceUncertainty", measure="entropy"
)
unc_result = []
...
for round_exp in range(10):
    ...
    # uncertainStrategy = alibox.get_query_strategy(
    #     strategy_name="QueryInstanceGraphDensity", train_idx=train_idx
    # )
...
analyser.plot_learning_curves(
    title="Example of alipy", std_area=False, saving_path="./Unc_Entropy"
)
